...
function OrderDetailContent() {
  ...
  const decisionMutation = useMutation({
    ...
    onError: () => {
      toastError("Failed to save decision");
    },
  });
  ...
  if (error || !record) {
    return (
      <div className="card p-8 text-center">
        ...
        <Link href="/orders/queue" className="btn-secondary mt-4 inline-flex">
          ← Back to Queue
        </Link>
      </div>
    );
  ...
  const { score } = verification.overall;
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between mt-3">
          <div>
            ...
            <p className="text-slate-500 text-sm font-mono mt-0.5">
              {record.id} · Created {formatDate(record.createdAt)}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mb-4">
              <div>
                ...
                <p className="font-medium text-slate-800">{order.contact.email}</p>
              </div>
              <div>
                <p className="text-slate-400 text-xs">Phone</p>
                <p className="font-medium text-slate-800">{order.contact.phone}</p>
              </div>
              <div>
                <p className="text-slate-400 text-xs">Billing Address</p>
              ...
              <div>
                <p className="text-slate-400 text-xs">Shipping Address</p>
                <p className="font-medium text-slate-800">
              ...
              {order.paymentMeta.cardLast4 && (
                <div>
                  ...
                  <p className="font-medium font-mono text-slate-800">
                    {order.paymentMeta.brand} ···{order.paymentMeta.cardLast4}
                  </p>
                </div>
              )}
              <div>
          ...
          {/* Claude Assist */}
          <ClaudeSummary order={order} verification={verification} />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-4">
            ...
            <div className="space-y-2">
              <button
                onClick={() => setDecisionModal({ open: true, initialStatus: "approved" })}
                className="btn-success w-full"
              >
                Approve Order
              </button>
              <button
                onClick={() => setDecisionModal({ open: true, initialStatus: "denied" })}
                className="btn-danger w-full"
              >
                Deny Order
              </button>
              <button
                onClick={() => setDecisionModal({ open: true, initialStatus: "queued" })}
                className="btn w-full bg-amber-500 text-white hover:bg-amber-600 focus:ring-amber-400"
              >
                Add Note / Re-queue
              </button>
            </div>
